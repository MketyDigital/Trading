...
## MT5 broker-comment fix — merged
PR #80 (`Fix MT5 broker comment compatibility`) is merged into `main`.
Merge commit: `5dd2a36368b3a1698ead62e0a218ff75895c7a33`.
...
Merged fix:
- Broker marker changed from the restrictive 31-char `mkety:` format to deterministic ASCII/alphanumeric <=20 chars: `mkety` + SHA prefix.
- Deterministic command identity/reconciliation is retained while avoiding restrictive MT5 comment validation.

Next verification:
- Confirm post-merge CI/deploy/release status.
- Run/restart the newly released MT5 connector build containing the merged bridge.
- Repeat controlled DEMO execution and confirm MT5 returns real broker IDs.
...
Checklist:
- [x] Broker-safe deterministic comment marker for the confirmed restrictive-comment failure.
- [ ] Fallback strategy for brokers that reject comments entirely.
- [ ] Symbol resolution/aliases/suffixes/prefixes.
...
### MT5 DEMO
Those entry events reached MT5 but failed at broker preflight because of the invalid comment format. The comment fix is now merged and must be re-tested with the fresh connector release.
...
- [ ] Required CI/deploy/release checks green on merged main.
...
1. Verify post-merge PR #80 CI, production deploy and fresh MT5 connector release.
2. Restart the MT5 connector with that release and rerun controlled DEMO entry acceptance.
3. Create a fresh stabilization branch from current `main`.
4. Implement source-agnostic DO -> Supabase position group/leg/broker-binding materialization under tests.
5. Add durable correlation recovery from Supabase and management lifecycle persistence.
6. Extend MT5 capability adaptation only where tests/DEMO expose real compatibility gaps (comments, symbol, volume, filling, stops, execution/order modes); do not hardcode broker vendors.
7. Run explicit Telegram REPLY management acceptance on XAUUSD (BE/SL update then close) across both DEMO brokers.
8. Run idempotency/reconnect acceptance.
9. Only after every DEMO gate above is green, perform a separate LIVE-readiness review. Do not enable LIVE automatically.
