...
if (!text.includes('data-source-enable')) {
  ...
  const helpers = `async function sourceLifecycle(id,action){var source=state.sources.find(function(x){return String(x.id)===String(id)});var body=action==='default'?JSON.stringify({sourceFamily:source&&source.sourceFamily||''}):'{}';await api('/api/v1/admin/sources/'+encodeURIComponent(id)+'/'+action,{method:'POST',body:body});await loadAll()}\nasync function accountLifecycle(id,action,enabled){await api('/api/v1/admin/accounts/'+encodeURIComponent(id)+'/'+action,{method:'POST',body:JSON.stringify({enabled:enabled})});await loadAll()}\nasync function destinationLifecycle(id,enabled){await api('/api/v1/admin/destinations/'+encodeURIComponent(id)+'/'+(enabled?'enable':'disable'),{method:'POST',body:'{}'});await loadAll()}\nasync function routeLifecycle(id,enabled){await api('/api/v1/admin/routes/'+encodeURIComponent(id)+'/'+(enabled?'enable':'disable'),{method:'POST',body:'{}'});await loadAll()}\n`;
...
// Avoid the historical generated-script newline parse regression.
text = text.replace("alert('Copy this one-time MTProto endpoint now:\\\n'+endpoint)", "alert('Copy this one-time MTProto endpoint now: '+endpoint)");
text = text.replace("alert('Copy this one-time signing secret now:\\\n'+secret)", "alert('Copy this one-time signing secret now: '+secret)");
