...
17. **Heterogeneous source provider configuration/coexistence validation — GREEN.** `src/sources/provider_config_validation.js` is a pure, side-effect-free validation boundary for one source record at a time. It requires provider/family consistency and a stable non-secret `external_identity` for canonical scoping, validates Container MTProto `chat_ids`, validates external MTProto server chat policy without allowing transport metadata to weaken it, and deliberately leaves MT5/cTrader/TradingView/custom broker/destination/execution concerns outside source configuration. `tests/multi_source_coexistence.test.mjs` proves Container MTProto, external MTProto, MT5 source bridge, cTrader source, TradingView webhook, and custom signed API can coexist in one workspace; one invalid provider does not invalidate or mutate siblings/defaults; validation output does not copy broker account or execution-enablement fields. TDD RED `33626475474` @ `7eaaac74bdd8d76a5b1522d63f36d447d0058cac` (only missing validation module); full GREEN `33626665176` @ `01e388ac6d94e2770461d9264340ec1fca7a3ab8` across Node, MT5, Container + external MTProto Python, and Wrangler gates.
...
1. Finish the remaining **Task 7 source registration/coexistence glue** from `docs/superpowers/plans/2026-09-02-multi-source-provider-foundation.md`, using the new pure provider validator rather than introducing provider-global state.
...
Re-read **Task 7** in `docs/superpowers/plans/2026-09-02-multi-source-provider-foundation.md` from the current branch and continue from the first unchecked step after provider configuration validation. Reuse `validateProviderConfiguration()` as the source-only validation boundary. Any registration/store/admin glue must validate one source independently, must not make one invalid source a workspace-wide failure, must never copy broker/destination/execution state into source authority, and must preserve per-family default semantics already enforced by `source_connection_store`. Write the next behavior as RED first, confirm the exact failure, implement only the smallest missing production seam, run all four gates, then update this `AGENTS.md` again before proceeding.