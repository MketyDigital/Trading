...
17. **Heterogeneous source provider registration/configuration/coexistence — GREEN.** `src/sources/provider_config_validation.js` is a pure, side-effect-free validation boundary for one source record at a time. It requires provider/family consistency and a stable non-secret `external_identity` for canonical scoping, validates Container MTProto `chat_ids`, validates external MTProto server chat policy without allowing transport metadata to weaken it, and deliberately leaves MT5/cTrader/TradingView/custom broker/destination/execution concerns outside source configuration. `tests/multi_source_coexistence.test.mjs` proves Container MTProto, external MTProto, MT5 source bridge, cTrader source, TradingView webhook, and custom signed API can all validate independently in one workspace; one invalid provider does not invalidate or mutate siblings/defaults; validation output does not copy broker account or execution-enablement fields. The same suite now exercises the real `source_connection_store` and proves all six enabled providers list together, disabled/other-workspace rows stay out, and defaults remain family-scoped with two simultaneous Telegram providers. TDD RED `33626475474` @ `7eaaac74bdd8d76a5b1522d63f36d447d0058cac` (only missing validation module); production GREEN `33626665176` @ `01e388ac6d94e2770461d9264340ec1fca7a3ab8`; store-level acceptance GREEN with no additional production change `33626927619` @ `a78dc9fbe257f41b67337ad5f06587a018f1d573`, all four mandatory gates passing. Broader-plan Task 7 is complete.
...
1. Harden **Task 6: Pure Durable Object MTProto alternate provider** without making it required/default and without leaking raw session strings from normal status/control responses.
2. Add Zitadel-authorized source admin/default/status APIs (Task 8).
3. Add non-live multi-source acceptance, reconnect/soak harness, runbook/CI expansion (Task 9).
4. External staging configuration and signed V1 + MTProto soak acceptance when runtime credentials are available.
5. Broker demo probes/lifecycles only behind explicit demo gates.
6. Tiny controlled live only after all acceptance is green.

## Exact next safe starting point

Continue `docs/superpowers/plans/2026-09-02-multi-source-provider-foundation.md` at **Task 6: Harden Pure Durable Object MTProto as an Alternate Provider**. First inspect current `src/listener/listener_node.js`, current mtcute/Cloudflare Durable Object runtime guidance, and any existing control-route callers. Write `tests/mtproto_do_provider.test.mjs` RED covering persisted mtcute session/update state, catch-up configuration, alarm-based reconnect, disconnected health, queue/signed-V1-compatible native event handoff, provider-independent Telegram canonical identity, and no raw session string in normal status/control responses. Do not make this provider a platform-wide startup dependency or default. Confirm the exact RED before modifying production listener/adding `src/sources/mtproto/do_provider.js`; after the next meaningful GREEN checkpoint, update this `AGENTS.md` again before proceeding.