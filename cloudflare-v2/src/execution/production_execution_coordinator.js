...
function safeMark(latencyTrace, name) { try { latencyTrace?.mark?.(name); } catch {} }
...
function accountEnvironment(account = {}) { return text(account.environment ?? account.account_environment ?? account.accountEnvironment).toLowerCase(); }
...
function hasBindableBrokerResult(result = {}) {
  return result?.brokerPositionId != null || result?.brokerOrderId != null || result?.brokerDealId != null || Number.isFinite(Number(result?.fillPrice));
}
...
function validateAccountAuthority(account, workspaceId, requestedAccountId, { liveBrokerExecutionEnabled = false } = {}) {
  ...
  if (environment && environment !== 'demo' && environment !== 'live') return blockedAccount(requestedAccountId, 'ACCOUNT_ENVIRONMENT_UNVERIFIED');
  if (environment === 'live') {
    if (liveBrokerExecutionEnabled !== true) return blockedAccount(requestedAccountId, 'LIVE_EXECUTION_GLOBAL_DISABLED');
    if (!accountLiveExecutionEnabled(account)) return blockedAccount(requestedAccountId, 'LIVE_EXECUTION_ACCOUNT_DISABLED');
  }
  return null;
}

async function runAccountPlan({ workspaceId, eventId, plan, accountLoader, authorityLoader, snapshotLoader, riskMaterializer, dispatchAction, stateBinder, bindingRepairRecorder, latencyTrace, liveBrokerExecutionEnabled }) {
  const requestedAccountId = text(plan?.accountId);
  if (!requestedAccountId) return blockedAccount('', 'ACCOUNT_ID_REQUIRED');
  let account;
  try { account = await accountLoader(workspaceId, requestedAccountId); }
  catch { return failedAccount(requestedAccountId, 'ACCOUNT_LOAD_FAILED'); }
  const initialAuthorityBlock = validateAccountAuthority(account, workspaceId, requestedAccountId, { liveBrokerExecutionEnabled });
  ...
  for (const action of actions) {
    ...
    if (typeof authorityLoader === 'function') {
      ...
      const currentAuthorityBlock = validateAccountAuthority(currentAccount, workspaceId, requestedAccountId, { liveBrokerExecutionEnabled });
      if (currentAuthorityBlock) return currentAuthorityBlock;
    }

    let snapshot = null;
    if (typeof snapshotLoader === 'function') {
      try { snapshot = await snapshotLoader({ workspaceId, eventId, sourceId: executionAuthority?.source?.id ?? null, account: currentAccount, action, plan }) ?? null; }
      catch { snapshot = null; }
    }
    ...
    if (typeof riskMaterializer === 'function') {
      try { materialized = await riskMaterializer({ workspaceId, eventId, groupId: plan?.groupId ?? null, account: currentAccount, action, plan, snapshot }); }
        ...
        outcomes.push({ status: 'BLOCKED', legId: action?.legId ?? null, idempotencyKey: action?.idempotencyKey ?? null, reason: error?.code || 'BROKER_RISK_CONTEXT_UNAVAILABLE' });
        continue;
      }
      if (materialized?.allowed === false) {
        outcomes.push({ status: 'BLOCKED', legId: action?.legId ?? null, idempotencyKey: action?.idempotencyKey ?? null, reason: materialized.reason || 'BROKER_RISK_BLOCKED' });
        continue;
      }
    ...
    if (!policy.allowed) {
      ...
      outcomes.push({ status: 'BLOCKED', legId: executableAction?.legId ?? null, idempotencyKey: executableAction?.idempotencyKey ?? null, policy });
      continue;
    }
    ...
    if (result?.duplicate !== true && hasBindableBrokerResult(result) && typeof stateBinder === 'function') {
      try {
        await stateBinder({ workspaceId, eventId, accountId: requestedAccountId, groupId: plan?.groupId ?? null, legId: executableAction?.legId ?? null, brokerPositionId: result?.brokerPositionId ?? null, brokerOrderId: result?.brokerOrderId ?? null, brokerDealId: result?.brokerDealId ?? null, fillPrice: Number.isFinite(Number(result?.fillPrice)) ? Number(result.fillPrice) : null });
      } catch {
        if (typeof bindingRepairRecorder === 'function') {
          try { await bindingRepairRecorder({ workspaceId, eventId, accountId: requestedAccountId, groupId: plan?.groupId ?? null, legId: executableAction?.legId ?? null, idempotencyKey: executableAction?.idempotencyKey ?? null }); } catch {}
        }
...
export async function executeProductionPlan({ workspaceId, eventId = null, accountPlans = [], brokerExecutionEnabled = false, liveBrokerExecutionEnabled = false } = {}, { accountLoader, authorityLoader, snapshotLoader, riskMaterializer, dispatchAction, stateBinder, bindingRepairRecorder, latencyTrace, finalizeExecutionBatch } = {}) {
  ...
  try {
    const accounts = await Promise.all(accountPlans.map((plan) => runAccountPlan({ workspaceId: trustedWorkspaceId, eventId, plan, accountLoader, authorityLoader, snapshotLoader, riskMaterializer, dispatchAction, stateBinder, bindingRepairRecorder, latencyTrace, liveBrokerExecutionEnabled })));
