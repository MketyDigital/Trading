...
try {
  ...
  try { await setExecution(false); console.log('DEMO_EXECUTION_GATE=OFF'); } catch (error) { console.error(`DEMO_GATE_ROLLBACK_FAILED=${error.message}`); process.exitCode = 2; }
  try {
    const c = await controls();
    const a = await account();
    ...
    if (c.live_broker_execution_enabled !== false || a?.execution_enabled !== false || a?.live_execution_enabled !== false || (live || []).some((x) => x.execution_enabled || x.live_execution_enabled)) { console.error('LIVE_SAFETY_POSTURE=FAIL'); process.exitCode = 2; } else console.log('LIVE_SAFETY_POSTURE=PASS');
