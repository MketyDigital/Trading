...
test('cTrader close does not depend on unavailable dynamic exposure context', async () => {
  let exposureCalls = 0;
...
test('risk-increasing open still fails closed when required exposure context is unavailable', async () => {
  let exposureCalls = 0;
  const deps = createLegacyDeps({
    env: { TRADING_MASTER_KEY: 'master' }, supabase: supabaseStub(), workspaceId: 'ws-a', tradingEventId: 'event-open',
  }, {
    exposureLoader: async () => { exposureCalls += 1; throw Object.assign(new Error('exposure unavailable'), { code: 'BROKER_RISK_CONTEXT_UNAVAILABLE' }); },
    deliveryStoreFactory: () => ({ reserve() {}, complete() {}, fail() {} }),
  });
...
test('MT5 BE checks fresh broker price without requiring dynamic exposure service', async () => {
  let exposureCalls = 0;
