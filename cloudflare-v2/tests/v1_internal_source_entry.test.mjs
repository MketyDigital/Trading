...
test('worker routes only exact internal source-event path to trusted handoff handler', async () => {
  const seen = [];
  ...
  const env = { INTERNAL_SOURCE_TRANSPORT_TOKEN: 'configured' };
  const ctx = { waitUntil() {} };
...
test('nearby internal paths remain closed to legacy routing', async () => {
  let legacyCalls = 0;
  const worker = createTradingV1Entrypoint({
