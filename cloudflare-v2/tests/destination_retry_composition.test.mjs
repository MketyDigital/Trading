...
test('due retry scan is bounded to RETRYABLE rows whose time is due, lease is free, and attempts remain', async () => {
  const rows = [{ id: 'd1', status: 'RETRYABLE' }];
  const { supabase, calls } = queryRecorder(rows);
