...
test('V1 simulation command defaults to signed complete signal then exact duplicate replay', async () => {
  const calls = [];
  const lines = [];
  ...
  assert.equal(calls[0].name, 'complete_signal');
  assert.equal(calls[1].name, 'duplicate');
...
test('V1 simulation command accepts explicit non-broker scenario matrix', async () => {
  const names = [];
  const result = await runV1SimulationAcceptanceCommand({
    env: {
      TRADING_V1_ENDPOINT: 'https://trade.example.test/api/v1/events',
      ...
      TRADING_V1_ACCEPTANCE_SCENARIOS: 'complete_signal,fast_entry,pending_order,ambiguous',
    },
    logger: { log() {}, error() {} },
    scenarioRunner: async ({ scenario }) => {
      names.push(scenario.name);
      return { ok: true, result: { scenario: scenario.name, externalEventId: scenario.event.external_event_id, response: { statusCode: 200, body: {} } } };
    },
  });

  assert.equal(result.ok, true);
  assert.deepEqual(names, ['complete_signal', 'fast_entry', 'pending_order', 'ambiguous']);
