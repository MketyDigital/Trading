...
#### Seven-day public live-observation checkpoint — 2026-09-19

- Owner decision: keep `MketyDigital/Trading` PUBLIC for approximately seven initial LIVE observation days before any visibility change. Earliest intended private review/cutover date: 2026-09-26. Do not change repository visibility before owner explicitly asks.
- Private-readiness work remains complete and reversible. Production OCI gateway is already the authenticated Coolify GitHub App source app `wp23orxgfa9py7giponnvcjt` (`mkety-github`, source id 1). Old public-source OCI app `p9xqtqpbljnggchy36mzd7a0` remains stopped rollback.
- PR #118 private-repository operations documentation merged successfully. Repository remains public.
- Fresh public production verification at ~2026-09-19 15:34 UTC: `cbot.mkety.com` resolved to OCI `89.168.70.209`; `/health` passed; unauthenticated WebSocket probes `/v1/cbot` and `/v1/mt5` both passed.
- Fresh authenticated gateway session verification: MT5 DEMO account `213921698` / `OctaFX-Demo` online with fresh heartbeat; FBS MT5 LIVE `110664480` / `FBS-Real` online with fresh heartbeat on OCI. Azure direct-origin registries show both MT5 accounts offline there, confirming sessions are on OCI.
- cTrader DEMO and cTrader LIVE authenticated gateway sessions were offline at the same verification point. This is not caused by private-source migration; it means the corresponding cBots are not currently connected/running.
- Global runtime controls remain intentionally owner-enabled: `trading_access_enabled=true`, `broker_execution_enabled=true`, `live_broker_execution_enabled=true`.
- FBS MT5 LIVE account remains active, execution ON, LIVE execution ON, fixed lot `0.01`, connector status connected.
- Recent execution audit found no FBS LIVE/private-source infrastructure failure. Two recent failures (`telegram:-1001822170589:24270`, `24271`) were cTrader DEMO management messages `TP 1` / `TP 2` returning terminal `POSITION_NOT_FOUND` while the cTrader cBot was offline/stale. These are separate cTrader state/session issues and should not be treated as private-repository or OCI-gateway failures.
- Current cTrader persisted state still contains multiple historical groups marked OPEN while broker-side positions may no longer exist. Do not bulk-close or mutate these rows blindly; reconcile against a connected cTrader broker session before cleanup.
...
#### Universal broker catalog execution + MT5 startup posture — 2026-09-19

- PR #119 merged as `b0c85a871d2d80ba8809cd9ff21befadb24c360c`: cTrader protocol min/max/step volume is normalized to user-facing lots at catalog ingestion/sanitization boundaries; MT5 lot semantics remain unchanged.
- PR #120 merged as `bc84a6a5cc2cc271ee0170fee7ebdbc9bf29592d`: broker account catalog is authoritative for destination symbol resolution, source symbol is used as a fallback when canonical naming differs, and fixed-lot preferences are materialized to broker min/max/step constraints instead of being rejected solely for benign lot granularity differences.
- Production Cloudflare Deploy #105 passed after PR #120, including dry-run, Worker deploy, production health, persisted owner-switch verification and cleanup.
- Fresh `buy v75` after PR #119 produced a real cTrader DEMO broker position; `v75(1s)` interpretation was also correct. Its earlier non-execution was caused by broker minimum 0.05 versus configured 0.01, now addressed generically by PR #120.
- Global controls remain owner-enabled. FBS MT5 LIVE remains active/execution/live-execution ON at fixed 0.01. cTrader DEMO remains active at fixed 0.01. cTrader LIVE remains intentionally inactive/off and was not changed.
- MT5 connector process already has its own WebSocket reconnect loop with three-second retry, heartbeat and symbol refresh. Do not add a second resident watchdog by default.
- Remaining MT5 Windows resilience gap is process/Windows restart. Added `mt5-connector/windows/Register-MketyMT5ConnectorStartupTask.ps1` plus `docs/MT5_CONNECTOR_WINDOWS_OPERATIONS.md` to register the existing connector with Windows Task Scheduler at interactive logon and restart it if the connector process exits. This adds no second always-running Mkety watchdog process.
- Repository remains PUBLIC during the agreed observation period and is still private-ready. Production Coolify is already on authenticated GitHub App source app `wp23orxgfa9py7giponnvcjt`; old public-source app is stopped rollback.
- Added `docs/AZURE_SELF_HOSTED_GITHUB_RUNNER.md`. Next infrastructure task is the isolated Azure Ubuntu organization-level runner with labels `self-hosted, linux, x64, mkety-ci`, kept separate from OCI production and Windows broker VPS machines.
